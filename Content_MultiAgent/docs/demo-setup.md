...
6. For LP, edit `Replacement statement` and click `Apply correction`.
...
the same LP job validates one operator-authored replacement statement, writes it to
`revision_history.json`, snapshots the rejected draft as `english_original_rejected.md`
and reaches an approved final package. Common numerical or promotional-result
patterns are rejected before any correction artifacts are written. The UI displays the rejected and
